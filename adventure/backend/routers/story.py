...
def get_session_id(session_id: Optional[str] = Cookie(None)):
    if not session_id:
        session_id = str(uuid.uuid4()) # if no session id exists, creates a new session
    return session_id
...
def generate_story_task(job_id: str, theme: str, session_id: str):
    db = SessionLocal()# need a separate database, to prevent hanging when accessing db and API accessing it
