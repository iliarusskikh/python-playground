#uvicorn nj_advanced_server:app --reload --port 8000
#localhost:8000/docs
#or http://127.0.0.1:8000/warehouse/tomatoes?order_qty=30
#source .venv/bin/activate

#curl -X GET 'http://127.0.0.1:8000/warehouse/tomatoes?order_qty=50'
...
async def load_truck(product, order_qty):
    ...
    if int(order_qty) < int(0):
        # don't process the order - raise an exception
        raise HTTPException(
            status_code=400,
            detail=f"Sorry, order quantity has to be a positive number, please try again…"
        )
